...
export function Addprojectpopup({ show, close, projectLists,unSelectedProjectedData }: any) {
    const [selectedProjects, setSelectedProjects] = React.useState<
        { projectid: string; projectname: string }[]
    >([]);

    console.log('selectedProjectsId',unSelectedProjectedData);
    ...
    const handleCheckboxChange = (
        ...
        console.log(
            ...
            isChecked
                ...
                : selectedProjects.filter((p) => p.projectid !== project.projectid)
        );
    };
                        ...
                        <div className="row px-2">
                            <div className="col-12">
                                <div className="summary py-1 d-flex justify-content-between align-items-center">
                                    <h5 className="heading2 me-3 textheader mb-0">Add projects</h5>
                                    <FontAwesomeIcon
                                        className="my-2"
                                        icon={faXmark}
                                        onClick={close} />
                            ...
                            >
                                {unSelectedProjectedData?.map((project:any, index:number) => (
                                    <div key={index} className="pb-2">
                                        <Commoncheckbox
                                            selectdata={project}
                                            onChange={(isChecked: boolean) => handleCheckboxChange(project, isChecked)} />
                                    </div>
                                ))}
                            </div>
