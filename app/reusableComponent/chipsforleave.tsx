...
const chipsColor = [
  ...
  { name: "Casual Leave", color: "#FFBA27", background: "rgba(255, 180, 0, 0.20)" },
  { name: "Sick Leave", color: "#FFBA27", background: "rgba(255, 180, 0, 0.20)" },
...
export default function ChipsForLeave({ label }: chipsProps) {
  const [chipStyle, setChipStyle] = useState<{ color: string; background: string } | null>(null);
  ...
  useEffect(() => {
    if (label !== "") {
      ...
      setChipStyle(style ? { color: style.color, background: style.background } : null);
